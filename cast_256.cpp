...
#define rotl32(x, n)   (((x) << ((uint32_t)(n & 31))) | ((x) >> (32 - (uint32_t)(n & 31))))
#define rotr32(x, n)   (((x) >> ((uint32_t)(n & 31))) | ((x) << (32 - (uint32_t)(n & 31))))
...
#define k_rnd(k, tr, tm)          \
    ...
    cast_256::cast_256() {
        ...
        cast::_mcrypt_set_key((cast256_key *)(this->key), (uint32_t * )
        this->keyword, this->keyword_size);
    }

    cast_256::cast_256(char *keyword, int32_t keyword_size, char *text, uint64_t text_size) {
        key = new void *[sizeof(cast::cast256_key)];
        this->keyword = keyword;
        if (this->keyword == nullptr)
            exit(1);
        this->keyword_size = keyword_size;
        this->text = text;
    ...
    cast_256::cast_256(char *keyword, const int32_t &keywordSize, std::vector<char> &buffer) {
        key = new void *[sizeof(cast::cast256_key)];
        this->keyword = keyword;
        if (this->keyword == nullptr)
            exit(-1);
    ...
    cast_256::~cast_256() {
        delete[](cast256_key *) (key);
//        delete[](keyword);
    }

    void cast_256::setKeyword(char *keyword, int32_t keyword_size) {
        this->keyword_size = keyword_size;
        if (keyword == nullptr)
            return;
        cast_256::keyword = keyword;
        generate_key();
    }

    void cast_256::setText(char *text) {
        if (text == nullptr)
            return;
        cast_256::text = text;
    }

    void cast_256::setTextSize(uint64_t textSize) {
        text_size = textSize;
    }

    char *cast_256::getKeyword() const {
        return keyword;
    }

    const int32_t cast_256::getKeywordSize() const {
        return keyword_size;
    }

    char *cast_256::getText() const {
        return text;
    }

    uint64_t cast_256::getTextSize() const {
        return text_size;
    }

    void *cast_256::getKey() const {
        return key;
    }

    int cast_256::generate_key() {
        if(this->keyword == nullptr)
            return -1;
        if(this->key == nullptr)
            return -2;
        int ret = cast::_mcrypt_set_key((cast256_key *)(this->key), (uint32_t * )
        this->keyword, this->keyword_size);
        return ret;
    }

    int cast_256::encrypt_64bytes(char* text) {
        if (text == nullptr)
            return -2;
        if (this->keyword == nullptr)
            return -3;
        if(generate_key() != 0)
            return -4;
        //0-15
        _mcrypt_encrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)text);
        //16-31
        _mcrypt_encrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)(&text[16]));
        //32-47
        _mcrypt_encrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)(&text[32]));
        //48-63
        _mcrypt_encrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)(&text[48]));

        return 0;
    }

    int cast_256::decrypt_64bytes(char* text) {
        ...
        //0-15
        _mcrypt_decrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)text);
        //16-31
        _mcrypt_decrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)(&text[16]));
        //32-47
        _mcrypt_decrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)(&text[32]));
        //48-63
        _mcrypt_decrypt(static_cast<cast::cast256_key *>(this->key), (uint32_t*)(&text[48]));
        return 0;
    }

    int cast_256::encrypt_data() {
        if (text_size == 0)
            return -1;
        if (text == nullptr)
            return -2;
        ...
        int i = 0;
        for (; i < text_size; i+=64) {
            if(encrypt_64bytes(&text[i]) != 0)
                return -6;
        }
    ...
    int cast_256::decrypt_data() {
        ...
        for (int i = 0; i < text_size; i+=64) {
            if(decrypt_64bytes(&text[i]) != 0)
                return -6;
        }
        return 0;
    }
    ...
    int _mcrypt_set_key(cast256_key *key, const uint32_t *in_key, const int key_len) {
